...
MODULE_TO_PACKAGE = {
    ...
    'click':'click','jinja2':'Jinja2','markupsafe':'MarkupSafe','werkzeug':'Werkzeug','clr':' clr_loader',
...
class PythonInstallWorker(QThread):
    ...
    def run(self):
        try:
            ...
            mirrors = [
                f"https://mirrors.tuna.tsinghua.edu.cn/python/{version}/",
                f"https://mirrors.aliyun.com/python/{version}/",
            ...
            for mirror in mirrors:
                ...
                url = mirror + filename
                try:
                    with urllib.request.urlopen(url, context=ssl_context, timeout=30) as r:
            ...
            if process.returncode == 0:
                self.finished_signal.emit(True, "Python 3.12.10 安装成功")
            else:
                self.finished_signal.emit(False, "安装失败")
...
def ensure_python_on_startup():
    """
    启动时检测 Python（完全异步，不阻塞主流程）
    有 Python 直接过，没有则后台静默安装，装完提示重启
    """
    if not getattr(sys, 'frozen', False):
        return True
    has_python, version = check_python_installed()
    if has_python:
        #print(f"[Main] Python {version}")
        return True
    # ===== 没有 Python =====
    # 检查管理员权限
    if sys.platform == "win32":
        try:
            ...
            if not ctypes.windll.shell32.IsUserAnAdmin():
                print("[Main] 权限不足，请以管理员身份运行")
                QMessageBox.warning(None, "权限不足", "请以管理员身份运行本程序")
                return False
        except:
            pass
    # 创建临时 QApplication（如果没有）
    ...
    # 创建隐藏的进度对话框（最小化提示）
    dialog = QDialog()
    dialog.setWindowTitle("安装 Python")
    ...
    layout.setSpacing(8)
    status_label = QLabel("⏳ 正在静默安装 Python 3.12.10...")
    ...
    def on_finished(success, msg):
        ...
        if success:
            status_label.setText("✅ 安装完成，请重启程序")
...
class SystemMonitorThread(QThread):
    ...
    def __init__(self):
        ...
        self._interval = 3000  # 3秒更新一次
        self._lhm_computer = None
        self._init_lhm()

    def _init_lhm(self):
        """初始化 LibreHardwareMonitor（从 tools下 LibreHardwareMonitor 加载）"""
        if sys.platform != 'win32':
            return
        try:
            import clr
            ...
            lhm_dir = os.path.join(base_dir, 'tools', 'LibreHardwareMonitor')
            # 如果 tools\LibreHardwareMonitor 不存在，尝试其他位置
            if not os.path.exists(lhm_dir):
                lhm_dir = os.path.join(base_dir, 'LibreHardwareMonitor')
            if not os.path.exists(lhm_dir):
                lhm_dir = os.path.join(os.getcwd(), 'tools', 'LibreHardwareMonitor')
            #print(f"🔍 LHM 目录: {lhm_dir}")
            if not os.path.exists(lhm_dir):
                #print(f"❌ LHM 目录不存在: {lhm_dir}")
                return
            # ===== 1. 将所有 DLL 添加到 PATH =====
            ...
            loaded_count = 0
            for dep in dep_dlls:
                dep_path = os.path.join(lhm_dir, dep)
                if os.path.exists(dep_path):
                    try:
                        ...
                        pass
                        #print(f"⚠️ 加载 {dep} 失败: {e}")
            # ===== 3. 加载 LHM 主 DLL =====
            lhm_dll = os.path.join(lhm_dir, 'LibreHardwareMonitorLib.dll')
            if not os.path.exists(lhm_dll):
                # 尝试子目录
                for subdir in ['net471', 'net48', 'net5.0', 'net6.0', 'net7.0', 'net8.0']:
                    sub_path = os.path.join(lhm_dir, subdir, 'LibreHardwareMonitorLib.dll')
                    if os.path.exists(sub_path):
                        lhm_dll = sub_path
                        break
            if not os.path.exists(lhm_dll):
                #print(f"❌ 找不到 LibreHardwareMonitorLib.dll")
                return
            try:
                clr.AddReference(lhm_dll)
                #print(f"✅ 加载 LHM DLL: {lhm_dll}")
            except Exception as e:
                #print(f"❌ 加载 LHM DLL 失败: {e}")
                return
            # ===== 4. 初始化 Computer =====
            ...
            self._lhm_computer.IsControllerEnabled = True
            self._lhm_computer.IsNetworkEnabled = True
            self._lhm_computer.IsStorageEnabled = True
            self._lhm_computer.IsPowerMonitorEnabled = True
            self._lhm_computer.Open()
            #print(f"✅ LHM 初始化成功 (加载了 {loaded_count} 个依赖)")
    ...
    def _get_temp_lhm(self):
        """通过 LibreHardwareMonitor 获取 CPU 温度（直接读取）"""
        ...
        try:
            ...
            for hardware in computer.Hardware:
                # ===== 先更新硬件数据 =====
                hardware.Update()
                is_cpu = hardware.HardwareType == HardwareType.Cpu
                # 遍历传感器
    ...
    def _get_temp_psutil(self):
        ...
        try:
            ...
            if not temps:
                return None
            # 优先检测的关键字
            priority_keys = ['coretemp', 'k10temp', 'zenpower', 'cpu_thermal', 'acpitz', 'pch_skylake']
    ...
    def _get_temperature(self):
        """获取温度：按优先级尝试多种方案"""
        # 1. 优先 LHM（Windows 最准确）
        temp = self._get_temp_lhm()
        if temp is not None:
            return f"{temp:.1f}°C"
        # 2. 其次 psutil（Linux/macOS）
        temp = self._get_temp_psutil()
        if temp is not None:
            return f"{temp:.1f}°C"
        # 3. 最后 wmic（Windows 备用）
    ...
    def _get_resource_color(self, percent, is_temp=False):
        """获取资源颜色（绿→橙→红）"""
        if is_temp:
            # 温度：65°C 以下绿色，65-80°C 橙色，80°C 以上红色
            if percent > 80:
                return "#F44336"  # 红色
            elif percent > 65:
                return "#FF9800"  # 橙色
            ...
            # CPU/内存：60% 以下绿色，60-80% 橙色，80% 以上红色
            if percent > 80:
                return "#F44336"  # 红色
            elif percent > 60:
                return "#FF9800"  # 橙色
    ...
    def run(self):
        ...
        while self._is_running:
            try:
                # CPU 使用率（interval=1 让 psutil 内部阻塞）
    ...
    def stop(self):
        ...
        if self._lhm_computer:
            try:
                ...
                pass
# ===== UpdateVisitor 类（必须在 SystemMonitorThread 外部定义） =====

class UpdateVisitor:
    """更新硬件数据的访问者 - 正确实现 IVisitor 接口"""
...
class AboutDialog(QDialog):
    ...
    def should_show_dev_mode(self):
        ...
        if '__compiled__' in globals():
            return False
        # 3. py2exe / cx_Freeze / PyInstaller 通用
        if getattr(sys, 'frozen', False):
            return False
        # 4. 检查可执行文件后缀（Nuitka 编译的 .exe 没有 .py 源码）
        exe_path = sys.argv[0]
...
def get_exe_directory():
    # PyInstaller / Nuitka onefile / Nuitka standalone：sys.argv[0] 始终指向原始 exe
...
class PackageWorker(QThread):
    ...
    def _build_command(self):
        ...
        if packer == 'Nuitka':
            ...
            upx_path = cfg.get('upx_path', '')
            compress_level = cfg.get('compress_level', '默认')
            # 将UPX目录加入PATH（Nuitka找upx.exe用）
            upx_dir = os.path.dirname(upx_path)
            current_path = os.environ.get('PATH', '')
            if upx_dir and upx_dir not in current_path.split(os.pathsep):
                os.environ['PATH'] = upx_dir + os.pathsep + current_path
            if optimize == "速度优先":
                upx_enabled = False
                self.safe_log("⚡ 速度优先模式：禁用UPX压缩")
            else:
                upx_enabled = bool(upx_path and os.path.exists(upx_path) and compress_level != '不压')
            if upx_enabled:
                # Windows短路径转换（避免中文/空格导致Nuitka解析失败）
                if sys.platform == 'win32':
                    try:
                        import ctypes
                        buf = ctypes.create_unicode_buffer(260)
                        ctypes.windll.kernel32.GetShortPathNameW(upx_path, buf, 260)
                        if buf.value:
                            upx_path = buf.value
                    except Exception:
                        pass
...
class VersionInfoDialog(QDialog):
    ...
    def _parse_version_file(self, file_path):
        ...
        try:
            ...
            QMessageBox.warning(self, "导入失败", f"解析失败: {e}")
...
kids=[
    ...
    VarFileInfo([VarStruct(u'Translation', [1033, 1200])])
        ...
        try:
            ...
            QMessageBox.warning(self, "保存失败", f"无法保存版本文件:\n{e}")
        return path
...
class IconMakerDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        text_layout.addWidget(QLabel("输入文字生成简单图标（需Pillow库）"))
    ...
    def _generate(self):
        """生成或保存图标"""
        if self.mode == "text":
            self._generate_text_icon()
        else:
            self._save_image_icon()
    ...
    def _save_image_icon(self):
        """保存图片处理图标"""
        if not self.pil_img:
            QMessageBox.warning(self, "提示", "请先打开图片")
            return
        self._save_icon(self.pil_img)

    def _save_icon(self, img):
        """保存图标文件"""
        file_path, _ = QFileDialog.getSaveFileName(self, "保存图标", "icon.ico", 
...
class InjectSelectorDialog(QDialog):
    def __init__(self, parent=None, selected=None):
        ...
        self.selected = selected or {'single_instance': False, 'workdir': False, 'resource_path': False}
        layout = QVBoxLayout(self)
...
class PyOxidizerWorker(QThread):
    ...
    def _fix_cargo_toml_in_worker(self, cargo_toml_path):
        ...
        try:
            ...
            new_lines = []
            for line in lines:
                # 修复 edition = 2021 为 edition = "2021"
                # 匹配 edition = 数字（不带引号）的情况
                stripped = line.strip()
...
class InstallDepsThread(QThread):
    ...
    def _get_installed_packages00(self, python_exe):
        """获取指定Python环境的已安装包列表（强制使用传入的python_exe）"""
        if not python_exe or not os.path.exists(python_exe):
            self.log_signal.emit(f"⚠️ Python路径无效: {python_exe}")
            return set()
        try:
            import subprocess
            env = os.environ.copy()
            ...
            if result.returncode == 0 and result.stdout.strip():
                data = json.loads(result.stdout)
                installed = {item['name'].lower() for item in data}
                sample = list(installed)[:10]
                self.log_signal.emit(f"📋 已经安装 {len(installed)} 个包")
                return installed
            else:
                self.log_signal.emit(f"⚠️ pip list 失败: {result.stderr[:100] if result.stderr else '未知错误'}")
        except subprocess.TimeoutExpired:
            self.log_signal.emit("⚠️ 获取包列表超时")
        except Exception as e:
            self.log_signal.emit(f"⚠️ 获取包列表失败: {e}")
        return set()
        ...
        try:
            ...
            for i, pkg in enumerate(missing_packages):
                ...
                try:
                    ...
                    # 先尝试镜像源
                    result = subprocess.run(
                        [self.venv_python, '-m', 'pip', 'install', pkg, '-i', MIRROR, '--no-warn-script-location'],
                        capture_output=True, text=True,
                        env=env,
                        startupinfo=startupinfo,
                        creationflags=creationflags,
                    ...
                    if result.returncode == 0:
                        ...
                        # 尝试官方源
                        self.log_signal.emit(f"   ⚠️ 镜像源失败，尝试官方源...")
                        result2 = subprocess.run(
                            [self.venv_python, '-m', 'pip', 'install', pkg, '--no-warn-script-location'],
                            capture_output=True, text=True,
                            env=env,
                            startupinfo=startupinfo,
                            creationflags=creationflags,
                        ...
                        if result2.returncode == 0:
                            self.log_signal.emit(f"   ✅ {pkg} 安装成功 (官方源)")
                            success_count += 1
                        else:
                            # ===== 尝试包名映射 =====
                            ...
                            if mapped_pkg != pkg:
                                ...
                                result2 = subprocess.run(
                                    [self.venv_python, '-m', 'pip', 'install', mapped_pkg, '-i', MIRROR, '--no-warn-script-location'],
                                    capture_output=True, text=True,
                                    env=env,
                                    startupinfo=startupinfo,
                                    creationflags=creationflags,
...
class PackageMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        QApplication.processEvents()
        # ===== 窗口显示后再加载数据 =====
        QTimer.singleShot(50, self._load_all_cached_data)  # 只保留这个
        # QTimer.singleShot(20, self._lazy_init)  # 删除，功能合并到下面
        ...
        #self.monitor_thread.log_signal.connect(self.safe_log)
            ...
            if hasattr(self, 'status_temp'):
                if temp_str:
                    ...
                    if match:
                        ...
                        self.status_temp.setText(f"🌡️{temp_str}")
                else:
                    self.status_temp.setText("🌡️ --°C")
                    self.status_temp.setStyleSheet("color: #999; font-weight: bold;")
            # ===== 综合告警 =====
            warnings = []
            if cpu_percent > 80:
                warnings.append("CPU高")
            if mem_percent > 80:
                warnings.append("内存高")
            if temp_value is not None and temp_value > 80:
                warnings.append(f"温度{temp_value:.1f}°C")
            if hasattr(self, 'status_label'):
                if warnings:
                    self.status_label.setText(f"⚠️ {' '.join(warnings)}")
    ...
    def _on_input_file_changed(self, text):
        ...
        if text and os.path.exists(text):
            ...
            if base_name:
                ...
                # ============================================
                self._analyze_used(text, auto_add=True)
                self._update_hidden_count()
                ...
                self._auto_load_tool_icon(text, base_name)
                if self.venv_mode.isChecked():
                    venv_python = self._get_venv_python()
                    if venv_python and os.path.exists(venv_python):
                        self._install_missing_deps_with_progress(venv_python, text)
            ...
            try:
                ...
                if hasattr(self, 'hidden_imports_list') and self.hidden_imports_list:
                    ...
                    if 'pyqt6' in imports_lower:
                        ...
                        detected = 'kivy'
                # ===== 如果 hidden_imports_list 为空，重新分析 =====
    ...
    def _init_ui(self):
        ...
        self.exclude_frame.setMaximumHeight(70)
        ...
        """)
        self.exclude_listbox.setMaximumHeight(80)
    ...
    def _show_excluded_packages_dialog(self):
        ...
        try:
            ...
            self.safe_log(f"⚠️ 分析导入失败: {e}")
        if not used_in_code:
            show_msg(self, "提示", "脚本中没有检测到第三方模块导入",1)
            return
        # 映射到包名
        used_packages = set()
        for mod in used_in_code:
            pkg = MODULE_TO_PACKAGE.get(mod, mod.lower())
            if pkg == 'pil':
                pkg = 'pillow'
            used_packages.add(pkg)
        # 构建排除列表：只显示代码中用到的但被排除的包
        exclude_list = []
        for pkg in used_packages:
            # 检查是否在排除列表中
            if pkg in self.exclude_list:
                exclude_list.append(pkg)
            # 检查是否在 hiddenimports 中（如果不在，说明被自动排除了）
            elif pkg not in [MODULE_TO_PACKAGE.get(m, m.lower()) for m in self.hidden_imports_list]:
                # 不在 hiddenimports 中，说明被自动排除了
                exclude_list.append(pkg)
        if not exclude_list:
            show_msg(self, "提示", "所有用到的包都已包含在打包中，没有需要恢复的",1)
            return
        # ===== 创建对话框 =====
        dialog = QDialog(self)
        ...
        layout = QVBoxLayout(dialog)
        info = QLabel(f"🟧 代码中用到了 {len(exclude_list)} 个包，但被排除了")
        info.setStyleSheet("color: #333; font-size: 12px; padding: 6px; background-color: #fff3cd; border-radius: 4px;")
        layout.addWidget(info)
        info2 = QLabel("💡 选中需要恢复的包，点击「恢复选中」即可加入打包")
        info2.setStyleSheet("color: #666; font-size: 10px; padding: 2px 4px;")
        ...
        """)
        for pkg in sorted(exclude_list):
            item = QListWidgetItem(f"📦 {pkg}")
            item.setBackground(QColor("#FF6B00"))
            item.setForeground(QColor("#000000"))
            item.setData(Qt.ItemDataRole.UserRole, pkg)
            list_widget.addItem(item)
        layout.addWidget(list_widget)
        # 统计
        count_label = QLabel(f"共 {len(exclude_list)} 个包需要恢复")
        ...
        """)
        btn_restore.clicked.connect(lambda: self._restore_selected_packages_fast(list_widget, dialog))
    ...
    def _restore_selected_packages_fast(self, list_widget, dialog):
        """快速恢复选中的包"""
        restored = []
    ...
    def _get_installed_packages(self, python_exe):
        """获取指定Python环境的已安装包列表"""
        if not python_exe or not os.path.exists(python_exe):
            self.safe_log(f"⚠️ Python路径无效: {python_exe}")
            return set()
        # ===== 判断虚拟环境 =====
        if hasattr(self, 'parent') and hasattr(self.parent(), 'use_venv'):
            if self.parent().use_venv:
               venv_python = self.parent()._get_venv_python()
               if venv_python and os.path.exists(venv_python):
                   python_exe = venv_python
                   self.safe_log(f"🔍 虚拟环境模式，强制使用: {python_exe}")
        try:
            import subprocess
            env = os.environ.copy()
            # 清空可能干扰的PYTHONPATH
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                installed = {item['name'].lower() for item in data}
                sample = list(installed)[:10]
                self.safe_log(f"📋  {python_exe} 获取到已安装 {len(installed)} 个包")
                return installed
            else:
                self.safe_log(f"⚠️ pip list 失败: {result.stderr[:100]}")
        except Exception as e:
            self.safe_log(f"⚠️ 获取包列表失败: {e}")
        return set()
    ...
    def _run_build(self):
        ...
        if script and os.path.exists(script):
            try:
                ...
                self.safe_log(f"⚠️ 分析脚本导入失败: {e}")
        self.safe_log(f"📦 代码中检测到 {len(used_in_code)} 个模块。包括: {', '.join(list(used_in_code)[:20])}" + (
            '...' if len(used_in_code) > 20 else ''))
        # ===== 获取所有已安装的包（用于排除） =====
        all_installed = set()
        pkg_python = target_python
        self.safe_log(f"🐍 使用Python获取包列表: {target_python}")
        all_installed = self._get_installed_packages(pkg_python)
        self.safe_log(
            f"📦 当前环境已安装 {len(all_installed)} 个包。包括: {', '.join(list(all_installed)[:20])}" + (
                '...' if len(all_installed) > 20 else ''))
        # ===== 合并 hiddenimports =====
        ...
        for mod in used_in_code:
            if mod not in STANDARD_LIBS:
                final_hidden_imports.add(mod)
        if 'PyQt6' in used_in_code or 'PyQt6' in self.hidden_imports_list:
            final_hidden_imports.add('PyQt6')
        if 'PyQt5' in used_in_code or 'PyQt5' in self.hidden_imports_list:
            final_hidden_imports.add('PyQt5')
        if 'PIL' in used_in_code or 'Image' in used_in_code or 'PIL' in self.hidden_imports_list:
            final_hidden_imports.add('PIL')
        if 'requests' in used_in_code in self.hidden_imports_list:
            final_hidden_imports.add('urllib3')
            final_hidden_imports.add('certifi')
        final_hidden_imports.discard('PyInstaller')
        final_hidden_imports.discard('pyinstaller')
        final_hidden_imports.discard('pyi_hooks_contrib')
        # ===== 确保这些变量是列表/集合，不是 None =====
        if final_hidden_imports is None:
            final_hidden_imports = set()
        if self.exclude_list is None:
            self.exclude_list = []
        if self.hidden_imports_list is None:
            self.hidden_imports_list = []
        if used_in_code is None:
            used_in_code = set()
        if filtered_data_files is None:
            filtered_data_files = []
        filtered_modules = set(used_in_code) - set(final_hidden_imports)
        filtered_modules = {m for m in filtered_modules if m not in STANDARD_LIBS}
        if filtered_modules:
            self.safe_log(f"📦 自动过滤 {len(filtered_modules)} 个模块（不需要打包）: {', '.join(filtered_modules)}")
        self.safe_log(
            f"📦 最终 hiddenimports: {len(final_hidden_imports)} 个。包括: {', '.join(list(final_hidden_imports)[:20])}" + (
                '...' if len(final_hidden_imports) > 20 else ''))
        # ===== 构建排除列表 =====
        needed_packages = set(final_hidden_imports)
        dependency_packages = {}
        if 'PyQt6' in final_hidden_imports:
            dependency_packages['PyQt6'] = ['pyqt6-qt6', 'pyqt6_sip']
        if 'PyQt5' in final_hidden_imports:
            dependency_packages['PyQt5'] = ['pyqt5-qt5', 'pyqt5_sip']
        if 'PIL' in final_hidden_imports:
            dependency_packages['PIL'] = ['pillow']
        all_dependency_packages = set()
        for deps in dependency_packages.values():
            for pkg in deps:
                all_dependency_packages.add(pkg)
        exclude_list = []
        for pkg in all_installed:
            is_stdlib = False
            for std_mod in STANDARD_LIBS:
                install_name = MODULE_TO_PACKAGE.get(std_mod, std_mod.lower())
                if install_name.lower() == pkg.lower():
                    is_stdlib = True
                    break
            if is_stdlib:
                continue
            is_needed = False
            for mod in needed_packages:
                if mod.lower() == pkg.lower():
                    is_needed = True
                    break
                install_name = MODULE_TO_PACKAGE.get(mod, mod.lower())
                if install_name.lower() == pkg.lower():
                    is_needed = True
                    break
            if is_needed:
                continue
            if pkg not in exclude_list:
                exclude_list.append(pkg)
        for pkg in all_dependency_packages:
            if pkg not in exclude_list:
                exclude_list.append(pkg)
        for mod in self.exclude_list:
            mod_lower = mod.lower()
            if mod_lower not in exclude_list:
                exclude_list.append(mod_lower)
        if 'requests' in final_hidden_imports:
            # requests 的附属依赖库
            requests_deps = [
                'urllib3',
                'certifi',
                'idna',
                'charset_normalizer',  # 或 charset-normalizer
                'chardet',  # 可选的
            ]
            # 从排除列表中移除这些依赖
            removed_count = 0
            for dep in requests_deps:
                if dep in exclude_list:
                    exclude_list.remove(dep)
                    self.safe_log(f"📦 保留 requests 附属依赖: {dep}")
                    removed_count += 1
                # 也检查带连字符的版本
                dep_hyphen = dep.replace('_', '-')
                if dep_hyphen != dep and dep_hyphen in exclude_list:
                    exclude_list.remove(dep_hyphen)
                    self.safe_log(f"📦 保留 requests 附属依赖: {dep_hyphen}")
                    removed_count += 1
            if removed_count > 0:
                self.safe_log(f"✅ 已从排除列表恢复 {removed_count} 个 requests 附属依赖")
        self.safe_log(f"📦 需要保留的包: {len(needed_packages)} 个。包括: {', '.join(list(needed_packages)[:20])}" + (
            '...' if len(needed_packages) > 20 else ''))
        exclude_count_for_log = len(all_installed) - len(needed_packages)
        self.safe_log(
            f"📦 已安装 {len(all_installed)} 个，保留 {len(needed_packages)} 个，排除 {exclude_count_for_log} 个（含附属依赖）")
        if dependency_packages:
            dep_msg = []
            for main_mod, deps in dependency_packages.items():
                dep_msg.append(f"{main_mod} → {', '.join(deps)}")
            self.safe_log(f"📦 附属依赖（已加入排除列表）: {'; '.join(dep_msg)}")
        self.safe_log(f"🚫 排除模块包括: {', '.join(exclude_list[:20])}" + (
    ...
    def _package_pyoxidizer0(self, input_file):
        """使用 PyOxidizer 打包"""
        import shutil
        import glob
        # ===== 获取 Python 路径 =====
        ...
        try:
            ...
            pass
        cargo_home = os.path.expanduser(r"~\.cargo")
        lock_file = os.path.join(cargo_home, ".package-cache")
        if os.path.exists(lock_file):
            try:
                os.remove(lock_file)
                self.safe_log("🧹 已删除 cargo 缓存锁")
            except:
                pass
        cargo_cache = os.path.expanduser(r"~\.cargo\.package-cache")
        if os.path.exists(cargo_cache):
            try:
                os.remove(cargo_cache)
                self.safe_log("🧹 已清理 cargo 缓存锁")
            except:
                pass
        self.safe_log("=" * 50)
        self.safe_log("📦 PyOxidizer 打包模式")
        ...
        self.safe_log(f"✅ 发行版: {dist_path}")
        # ===== 配置本地 Cargo 镜像 =====
        cargo_dir = os.path.join(out_dir, ".cargo")
        ...
        # ===== 使用 QThread =====
        self.progress_bar.setValue(25)
        self.progress_label.setText("25% - 编译中...")
        QApplication.processEvents()
        config = {
            'pyoxidizer': pyoxidizer,
            'out_dir': out_dir,
    ...
    '''
        ...
        # 保存为新的模块名
        with open(os.path.join(build_dir, f"{module_name}.py"), 'w', encoding='utf-8') as f:
            f.write(content)
        # ===== 生成 __init__.py =====
        with open(os.path.join(build_dir, "__init__.py"), 'w', encoding='utf-8') as f:
    ...
    def _install_packer_async(self, python_exe, packer_name):
        ...
        try:
            ...
            result = subprocess.run(
                [python_exe, '-m', 'pip', 'install', packer_name, '-i', MIRROR, '-q'],
                capture_output=True, text=True, timeout=300
            )
            if result.returncode == 0:
    ...
    def _batch_install_in_venv(self, venv_python, modules):
        ...
        for i, mod in enumerate(modules):
            ...
            try:
                result = self._run_hidden(
                    [venv_python, '-m', 'pip', 'install', pkg, '-i', MIRROR, '--no-warn-script-location'],
                    capture_output=True, text=True,
                    startupinfo=get_startupinfo()
                )
        ...
        try:
            ...
            if 'tk' in self.hidden_imports_list:
                if 'tk' not in installed_in_venv:
                    ...
                    try:
                        result = self._run_hidden(
                            [venv_python, '-m', 'pip', 'install', 'tk', '-i', MIRROR, '-q'],
                            capture_output=True, text=True,
                            startupinfo=get_startupinfo(),
                            timeout=180
                        )
                        if result.returncode == 0:
                            ...
                            self.safe_log(f"   ❌ tk 安装失败: {result.stderr[:200]}")
                    except Exception as e:
            ...
            if missing_packages:
                ...
                for i, pkg in enumerate(missing_packages):
                    ...
                    try:
                        result = self._run_hidden(
                            [venv_python, '-m', 'pip', 'install', pkg, '-i', MIRROR, '-q'],
                            capture_output=True, text=True,
                            startupinfo=get_startupinfo(),
                            timeout=180
                        )
                        if result.returncode == 0:
                            ...
                            self.safe_log(f"   ❌ {pkg} 安装失败: {result.stderr[:200]}")
                    except Exception as e:
    ...
    def _find_all_python(self):
        ...
        for base_dir in [script_dir, current_dir]:
            ...
            for path in venv_paths:
                if is_valid_python(path) and path not in found_paths:
        ...
        if sys.platform == 'win32':
            ...
            common_venv_exe = os.path.join(common_venv_dir, "bin", "python")
        if os.path.exists(common_venv_exe) and is_valid_python(common_venv_exe):
            if common_venv_exe not in found_paths:
        ...
        if sys.platform == 'win32':
                ...
                try:
                    ...
                    if w.returncode == 0:
                        for line in w.stdout.strip().split('\n'):
                            line = line.strip()
                            if is_valid_python(line) and line not in found_paths:
            ...
            for pattern in [r'C:\Python*', rf'C:\Users\{username}\AppData\Local\Programs\Python\Python*',
                ...
                for path in glob.glob(pattern):
                    if os.path.isdir(path):
                        exe = os.path.join(path, 'python.exe')
                        if is_valid_python(exe) and exe not in found_paths:
        ...
        # 4. 源码模式添加当前 Python
        if not getattr(sys, 'frozen', False):
            if is_valid_python(sys.executable) and sys.executable not in found_paths:
    ...
    def _on_python_path_changed(self,  new_path=None):
        #def _on_python_path_changed(self, new_path):
    ...
    def _analyze_used(self, script_path, auto_add=True):
        ...
        FILTER_MODULES = {
            ...
            'setuptools', 'pip', 'wheel', 'distribute', 'pkg_resources',
            ...
            'numpy', 'yaml', 'pyyaml', 'win32', 'pywin32','win32con',
        }
        result = []
        ...
        if uses_tkinter and 'tk' not in result:
            result.append('tk')
        # ===== 4. 自动补充隐式依赖 =====
        if 'openpyxl' in result and 'et_xmlfile' not in result:
            result.append('et_xmlfile')
        if 'requests' in result and 'urllib3' not in result:
            result.append('urllib3')
        if 'requests' in result and 'certifi' not in result:
            result.append('certifi')
        ...
        if auto_add:
            ...
            self._update_hidden_count()
            if result:
                self.safe_log(f"✅ 导入 {len(result)} 个模块: {', '.join(result)}")
        ...
        class InstallDepsThread(QThread):
            ...
            def run(self):
                ...
                for i, pkg in enumerate(self.packages):
                    self.status_signal.emit(f"安装 {pkg} ({i+1}/{total})")
                    self.log_signal.emit(f"   📥 安装 {pkg}...")
                    progress = int((i + 1) / total * 100)
                    self.progress_signal.emit(progress)
                    try:
                        import subprocess
                        result = subprocess.run(
                            [self.python_exe, '-m', 'pip', 'install', pkg, '-i', MIRROR, '--no-warn-script-location'],
                            capture_output=True, text=True,
                            env=clean_env,
                            timeout=300
                        )
                        if result.returncode == 0:
                            ...
                            self.log_signal.emit(f"   ❌ {pkg} 安装失败: {result.stderr[:100]}")
                    except subprocess.TimeoutExpired:
                        self.log_signal.emit(f"   ❌ {pkg} 安装超时")
                    except Exception as e:
                        self.log_signal.emit(f"   ❌ {pkg} 安装异常: {e}")
    ...
    def _check_tkinter_usage(self, script_path):
        """检测脚本是否真正使用了 tkinter - 只检测真实 import"""
        try:
    ...
    def _toggle_exclude(self):
        if hasattr(self, 'exclude_frame'):
            visible = not self.exclude_frame.isVisible()
            self.exclude_frame.setVisible(visible)
            self.exclude_btn.setText("▼ 排除" if visible else "▶ 排除")

    def _toggle_advanced(self):
        if hasattr(self, 'adv_frame'):
            visible = not self.adv_frame.isVisible()
            self.adv_frame.setVisible(visible)
            self.adv_btn.setText("▼ 数据" if visible else "▶ 数据")
    ...
    def _add_exclude(self):
        ...
        if text:
            for mod in text.split(','):
                mod = mod.strip()
                if mod and mod not in self.exclude_list:
    ...
    def _remove_exclude(self):
        ...
        if current >= 0:
            ...
            if item.text() in self.exclude_list:
                ...
                self.safe_log(f"📌 已从排除列表移除: {item.text()}")
            self._update_exclude_count()
    ...
    def _add_recommended(self):
        for pkg in EXCLUDE_PACKAGES:
            if pkg not in self.exclude_list:
                ...
                self.exclude_listbox.addItem(pkg)
        self._update_exclude_count()
    ...
    def _scan_and_add_resources(self, script_dir):
        ...
        # 只添加已经存在于 data_files 中的资源，避免盲目扫描
        # 或者通过用户选择添加
        pass
    ...
    def _on_venv_switch(self, state):
        ...
        if self.use_venv:
            ...
            QTimer.singleShot(10, self._do_disable_venv)
    ...
    def _restore_system_python(self):
        ...
        try:
            ...
            if current_py and current_py == self._format_path(venv_python):
                ...
                # 如果当前不是虚拟环境Python，但可能版本信息没更新，刷新一下
                if current_py and os.path.exists(current_py):
                    try:
                        result = self._run_hidden(
                            [current_py, '--version'],
    ...
    def _do_manage_venv(self):
        """管理虚拟环境（原版保留，调用 _do_manage_venv_common）"""
        try:
            venv_dir = os.path.join(self.current_dir, "common_venv")
            script = self.input_file.text()
            if not script or not os.path.exists(script):
    ...
    def _get_empty_lines_to_delete(self, content, lines):
        ...
        def is_special_block(stripped):
            """判断是否是特殊块（def main、if __name__等）"""
        ...
        def collect_deletions_for_items(items, is_module=False):
            ...
            for i in range(len(sorted_items) - 1):
                ...
                if isinstance(next_stmt, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                    ...
                    # if __name__ == '__main__' 前面：保留1个空行，删除多余的
                    if len(empty_lines) > 1:
                        for line_no in empty_lines[:-1]:
                            delete_empty_lines.add(line_no)
                else:
        ...
        # ===== 新增：删除def/class体第一行前的空行 =====
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                body = getattr(node, 'body', None)
                if body and len(body) > 0:
                    def_line = node.lineno  # def/class所在行号
                    first_stmt_line = body[0].lineno  # 体中第一语句行号
                    # 删除def行和第一语句行之间的所有空行
                    for line_idx in range(def_line, first_stmt_line - 1):
                        if line_idx < len(lines) and self._is_really_empty(lines[line_idx]):
                            delete_empty_lines.add(line_idx + 1)
        return delete_empty_lines

    def _get_need_insert_lines(self, content, lines):
        """获取需要插入空行的位置（顶层特殊块之间缺少空行时）"""
        insert_lines = set()
        try:
            tree = ast.parse(content)
        except SyntaxError:
    ...
    def _check_code_formatting(self, content, lines, file_path):
        ...
        # 第一部分：检查顶层块之间的空行（函数、类、if __name__）
        ...
        for i, node in enumerate(module_body):
            if is_special_top_block(node):
                ...
                top_blocks.append((node.lineno, end, node, i))
        # 检查顶层块之间的空行
        for idx in range(len(top_blocks) - 1):
            curr_lineno, curr_end, curr_node, curr_idx = top_blocks[idx]
            next_lineno, next_end, next_node, next_idx = top_blocks[idx + 1]
            # 统计之间的空行（跳过注释行）
            empty_lines = []
            has_comment = False
            for line_idx in range(curr_end, next_lineno - 1):
                if line_idx >= len(lines):
                    break
                stripped = lines[line_idx].strip()
                if stripped.startswith('#'):
                ...
                if self._is_really_empty(lines[line_idx]):
                    empty_lines.append(line_idx + 1)
            curr_name = get_block_name(curr_node)
            next_name = get_block_name(next_node)
            # 有注释时最多允许2行空行
            if has_comment:
                if len(empty_lines) > 2:
                    errors.append((next_lineno,
                                   f"【顶层块之间】{next_name} 前有多余空行（与{curr_name}之间有注释，空行{len(empty_lines)}行，超过2行）"))
                continue
            # 没有注释时，应该恰好1行空行
            if len(empty_lines) == 0:
                errors.append((next_lineno,
                               f"【顶层块之间】{next_name} 前缺少空行（与{curr_name}之间应有1行空行）"))
            elif len(empty_lines) > 1:
                errors.append((empty_lines[0],
                               f"【顶层块之间】{next_name} 前有 {len(empty_lines)} 个空行（与{curr_name}之间，应只有1行）"))
        # 第二部分：检查函数体/类体内部的空行
        ...
        def check_body_items(items, block_name, block_type):
            ...
            for i in range(len(sorted_items) - 1):
                ...
                if isinstance(next_stmt, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                    ...
                    if len(empty_lines) >= 1:
                        errors.append((empty_lines[0],
                                       f"【{block_type}体内部】{block_type} '{block_name}' 内，{prev_desc} -> {next_desc} 之间有多余空行 ({len(empty_lines)} 行)，应删除"))
    ...
    def _check_syntax(self):
        ...
        try:
            ...
            py_files = []
            if os.path.isdir(f):
                exclude_dirs = {"venv", "common_venv", "__pycache__", ".venv", "env", "dist", "build", "site-packages",
                                "Lib", "lib"}
            ...
            for i, py_file in enumerate(py_files):
                ...
                try:
                    with open(py_file, 'r', encoding='utf-8') as fp:
                        ...
                        if mixed_lines:
                            rel_path = os.path.relpath(py_file, self.current_dir) if os.path.isdir(
                                f) else os.path.basename(py_file)
                        ...
                        try:
                            ...
                            continue
                        # 通过所有检查
                        rel_path = os.path.relpath(py_file, self.current_dir) if os.path.isdir(f) else os.path.basename(
                            py_file)
                        file_details.append(
                            f"   ✅ {rel_path}: {line_count} 行, {func_count} 函数, {class_count} 类, {file_import_count} 导入")
            ...
            third_party_imports = [
                ...
                if not is_standard_module(imp) and imp not in EXCLUDE_IMPORTS
            ]
    ...
    def _resize_media_stack(self):
        ...
        btn_w = int(btn_h * 1.5)  # 宽度 = 高度的 1.5 倍
    ...
    def _toggle_fullscreen(self):
        ...
        if self.video_widget.isFullScreen():
            ...
            # 进入全屏
            self.video_widget.setParent(None)  # 从布局中脱离
    ...
    #def _toggle_help_about(self):
        #QMessageBox.about(self, "关于", f"Python代码打包工具\n版本: {BUILD_DATE}\nPyQt6 完整版\n\n支持: PyInstaller, Nuitka, PyApp, Py2exe, Cx_Freeze, Pynsist, PyOxidizer, Py2app, Auto-exe")
    ...
    def _create_venv_sync(self, venv_dir, venv_python):
        ...
        for pkg in packers:
            ...
            if check.returncode == 0:
                continue
            subprocess.run(
                [venv_python, '-m', 'pip', 'install', pkg, '-i', MIRROR, '-q', '--no-warn-script-location'],
                capture_output=True, text=True,
    ...
    def _create_venv_sync00(self, venv_dir, venv_python):
        """创建虚拟环境，升级pip，安装打包器，写入缓存，安全切换"""
        import subprocess
        import shutil
        import json
        system_python = None
        for i in range(self.python_path.count()):
            path = self.python_path.itemText(i)
            if path and os.path.exists(path) and 'common_venv' not in path.lower():
                system_python = path
                break
        if not system_python:
            for cmd in ['python', 'python3']:
                p = shutil.which(cmd)
                if p and 'common_venv' not in p.lower():
                    system_python = p
                    break
        if not system_python:
            return
        ...
        # 安装打包器
        packers = [
            'pyinstaller', 'nuitka', 'pyapp', 'py2exe',
            'cx-freeze', 'pynsist', 'pyoxidizer', 'py2app'
        ]
        clean_env = os.environ.copy()
        for var in ['PYTHONPATH', 'PYTHONHOME', 'VIRTUAL_ENV']:
            clean_env.pop(var, None)
        ...
        for pkg in packers:
            ...
            try:
                ...
                if res.returncode != 0:
                    subprocess.run(
                        [venv_python, '-m', 'pip', 'install', pkg, '-q', '--no-warn-script-location'],
                        capture_output=True, text=True,
                        env=clean_env,
                        timeout=300
                    )
        ...
        if self.venv_mode.isChecked():
            ...
            QMetaObject.invokeMethod(self, "_on_python_selected",
                                     Qt.ConnectionType.QueuedConnection)
    ...
    def _install_missing_deps_only(self, script_path):
        ...
        for pkg in missing:
            result = subprocess.run(
                [python_exe, '-m', 'pip', 'install', pkg, '-i', MIRROR, '--no-warn-script-location'],
                capture_output=True, text=True,
                env=clean_env,
                timeout=180
            )
            if result.returncode == 0:
                ...
                self.safe_log(f"   ❌ {pkg} 安装失败: {result.stderr[:100]}")
...
def main():
    ...
    try:
        ...
        # ===== 修正 APP_BASE_PATH（Nuitka onefile 保险） =====
